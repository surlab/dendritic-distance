...
def main():
    ...
    for current_data_dir, dirs, files in os.walk(data_path, topdown=False):
        branch_csv_path = os.path.join(current_data_dir, 'dendritic_distance', 'seperated_by_branch_point.csv')
        if not('dendritic_distance' in current_data_dir) and not(os.path.exists(branch_csv_path)):
                ...
                if shaft_roi:
                    ...
                    for i in range(len(dend_d_mat)):
                        for j in range(len(dend_d_mat)):
                            if not(dend_d_mat[i,j] - dend_d_mat[j,i]<.0001):
                    ...
                    for check, value_list in sanity_checks.items():
                        ...
                        global_sanity_checks[check][current_data_dir]= sanity_checks[check]
