...
def save_plot(spines, all_segments, current_data_dir, dmats):
    ...
    for segment_key, segment in all_segments.items():
      ...
      for endpoint in segment.end_points:
        ...
        plt.scatter(endpoint.end_point_xy[0], endpoint.end_point_xy[1],color='b',marker=m)

    for spine in spines:
        #plot the ROI in question
        plt.plot(spine.spine_roi[0,:],spine.spine_roi[1,:], color='y')
        #plot the center of the roi as an X
        plt.scatter(spine.spine_center_xy[0], spine.spine_center_xy[1], marker='x', color='y')
        #plot line from center to dendrite
        plt.plot([spine.spine_neck_xy[0], spine.spine_center_xy[0]], [spine.spine_neck_xy[1], spine.spine_center_xy[1]], color='y')
        #plot the location of the spine stem and the synapse
        plt.scatter(spine.spine_neck_xy[0], spine.spine_neck_xy[1], marker='o', color='y')
    ...
    try:
        ...
        dend_label = 'dendritic distance = '+ str(convert_pixels_to_um(dendritic_ds[i,j]))
        eu_label = 'euclidian distance = '+ str(convert_pixels_to_um(center_ds[i,j]))
        plt.plot([spines[i].spine_center_xy[0], spines[j].spine_center_xy[0]],
